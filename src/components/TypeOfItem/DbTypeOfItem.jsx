...
class DbTypeOfItem extends Component {
    ...
    constructor(props) {
        ...
        this.state = {
            columns: [
                { field: 'typeOfItemId', headerName: 'TypeOfItemId', width: 200 },
    ...
    addTypeOfItem() {
        window.location.href = "/addTypeOfItem";
    }
    ...
    editTypeOfItem() {
        if(this.state.currentRow != null){
            window.location.href = `/editTypeOfItem/${this.state.currentRow}`;
        } else{
            alert("Выберите поле");
        }
    }
    ...
    deleteSucces(id) {
        fetch(baseUrl + "/db/TypeOfItem/Delete/" + id, {
            method: "DELETE",
            headers: {
    ...
    deleteTypeOfItem() {
        if(this.state.currentRow != null){
            let isSucces = window.confirm(`Удалить тип с ${this.state.currentRow} id ?`);
            if(isSucces){
        ...
        return (
            <div style={{ height: 700, width: '100%' }}>
                <Button
                    ...
                    onClick={(event) => this.addTypeOfItem()}
